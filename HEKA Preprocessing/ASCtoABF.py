...
colnames = ['Time1', 'Trace1', 'Time2', 'Trace2']
debug = False
def dbg(string):
    if debug == True:
        print(string)
...
for index in parm_df.index:
    ...
    if parm_df.loc[index][3] == 'V-Clamp' and parm_df.loc[index][9] == 'V-Clamp':
        voltageclamp[index] = True
    else:
        voltageclamp[index] = False

# Insert string at position from end of string (e.g. before .abf)
def insert_str(string, insert, index):
    pos = len(string)-index
...
for index in parm_df.index:
    ...
    with open(infilepath[index], 'r', newline='') as f:
        reader = csv.reader(f, delimiter=' ', skipinitialspace=True)
        row1 = next(reader)
    if len(row1) <= 3: # If not dual recording:
        data = pd.read_csv(infilepath[index], sep='\s+', skipinitialspace=True, names=colnames[:2])
        # dbg(str('Original data: ', '\n', data))
        freq = int(1 / data['Time1'].iloc[1])  # Reciprocal first time step, e.g. sampling rate in Hz
        if voltageclamp[index] == False:
            data['Trace1'] *= volt_to_mvfactor  # To convert volts -> milli for volts -> millivolts
            # dbg(str('volt->mv data: ', '\n', data))
        elif voltageclamp[index] == True:
            data['Trace1'] *= amp_to_picofactor
            # dbg(str('amp->pA data: ', '\n', data))
        # Need to refactor to stack from longform to wide
        data['sweep'] = data.groupby('Time1').cumcount()
        # dbg(str('post-cumcount data: ', '\n', data))
        data = data.pivot(index='Time1', columns='sweep', values='Trace1')
        ...
        if voltageclamp[index] == False:
            data['Trace1'] *= volt_to_mvfactor  # To convert volts -> milli for volts -> millivolts
            data['Trace2'] *= volt_to_mvfactor
            # dbg(str('volt->mv data: ', '\n', data))
        elif voltageclamp[index] == True:
            data['Trace1'] *= amp_to_picofactor
            data['Trace2'] *= amp_to_picofactor
            # dbg(str('amp->pA data: ', '\n', data))
        # Need to refactor to stack from longform to wide
        data['sweep'] = data.groupby('Time1').cumcount()
        data1 = data[['Time1', 'Trace1', 'sweep']]
        data2 = data[['Time2', 'Trace2', 'sweep']]
        # dbg(str('post-cumcount data: ', '\n', data))
        data1 = data1.pivot(index='Time1', columns='sweep', values='Trace1')
        data2 = data2.pivot(index='Time2', columns='sweep', values='Trace2')
        # dbg(str('post-pivot data: ', '\n', data))
        # data = data.set_index('Time', drop=True)
        if debug is True: print('Reindexed data: ', '\n', data1, data2)
        # dbg(str('Hz: ', freq, 'Sweeps: ', data.shape[1], 'VoltageClamp: ', voltageclamp[index], 'File: ', infilepath[index]))
        data1 = data1.transpose().fillna(0)  # Flip so each sweep is row as pyabf expects
        data2 = data2.transpose().fillna(0)  # Flip so each sweep is row as pyabf expects
        # In partial sweeps, pivot produces NaN where there are no values. Sets these to 0
        data1 = np.array(data1)
        data2 = np.array(data2)
        # dbg(str('Final np.array data: ', '\n', data))
        if voltageclamp[index] == False:
            abfWriter.writeABF1(data1, insert_str(outfilepath[index], '_T1', 4), freq, units='mV')
            abfWriter.writeABF1(data2, insert_str(outfilepath[index], '_T2', 4), freq, units='mV')
        elif voltageclamp[index] == True:
            abfWriter.writeABF1(data1, insert_str(outfilepath[index], '_T1', 4), freq, units='pA')
            abfWriter.writeABF1(data2, insert_str(outfilepath[index], '_T2', 4), freq, units='pA')
            # If out of range value struct_pack error, check V-Clamp/C-Clamp for wrong multiplier
