...
if plotbool == 'y':
    ...
    indiv_cells_NaV = ggplot(data=ivdf,  mapping=aes(x='mV', y='NaV_papf', color='Genotype'))\
                + geom_point(size=0.2)\
    ...
    # Individual cells, day x cell
    indiv_cells_ss = ggplot(data=ivdf, mapping=aes(x='mV', y='Steady_papf', color='Genotype')) \
           + geom_point(size=0.2) \
           + facet_grid('day ~ Condition', space='free') \
           + theme_light() \
           + theme(aspect_ratio=1.5)
